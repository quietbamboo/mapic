...
- (void)showThumbnails {
  ...
  if (!_thumbsController) {
    if (URL) {
      ...
      _thumbsController = (TTThumbsViewController *)[[navigator viewControllerForURL:URL query:query] retain];
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (BOOL)canShowModel {
  return _photoSource.numberOfPhoto > 0;
}
...
- (void)moveToNextValidPhoto {
  if (_centerPhotoIndex >= _photoSource.numberOfPhoto) {
    // We were positioned at an index that is past the end, so move to the last photo
    [self moveToPhotoAtIndex:_photoSource.numberOfPhoto - 1 withDelay:NO];
...
- (NSInteger)numberOfPagesInScrollView:(TTScrollView*)scrollView {
  return _photoSource.numberOfPhoto;
}
